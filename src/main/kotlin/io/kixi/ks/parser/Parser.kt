...
class Parser(private val tokens: List<Token>) {
     ...
     * return it if so; throw [ParseError] with [message] otherwise.
     */
    ...
    internal fun canStartExpression(): Boolean = when (peek().type) {
        ...
        BACKTICK_STRING, CHAR_LITERAL, TRUE, FALSE, NIL, URL_LITERAL -> true
    ...
    /** Throw a [ParseError] at the current token's position. */
    internal fun error(message: String): Nothing {
        throw ParseError(message, peek().location)
    }

    /** Throw a [ParseError] at a specific [token]'s position. */
